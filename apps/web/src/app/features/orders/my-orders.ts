            ...
            <li>
              <a class="order" [routerLink]="['/orders', order.id]">
                <div class="order__top">
                  <span class="order__number code">{{ order.orderNumber }}</span>
                  <app-status-chip [status]="order.status" />
                </div>

                <p class="order__date">{{ deliveryDate(order.deliveryDate) }}</p>
...
export class MyOrders {
  ...
  /** Where it goes: company and team, as captured on the order. */
